...
class FaceDataset(Dataset):
    def __init__(self):
        ...
        self.transform = transforms.Compose([   # <-- put it later in test data set
    ...
    def get_data(self):
        csv_data = (row for row in pd.read_csv(CSV_PATH, delimiter=',').itertuples())
        lst = list()
        for data in csv_data:
            ...
            lst.append([{'image': key, 'age': values[0]/120, 'ethnicity': values[1]/10, 'gender': values[2]/2},
                        pxls_tensor])
        return lst
...
"""
            ...
            assert len(img_path) >= 1, "No images found."
